...
<Table
  ...
  data={[
    ["getTabInfo()","static", "- tab: string | TabValue", "获取tab信息TabValue"],
    ["setIndex()","public", "- index 选中序号\n- autoScroll 自动滚动", "选中某个Tab"],
    ["scrollToIndex()","public", "- params: ScrollIndexParams 滚动参数", "滚动到某一Tab"],
    ["scrollTo()","public", "- params: { x: number; y?: number; animated?: boolean } 滚动参数", "滚动到指定偏移值"],
  ]}
/>
```
...
<View>
  ...
  <Tabs
    ...
    badgeList={[
      ...
      {index: 3, view: <HiText style={{marginLeft: 0,borderWidth:0.5,borderColor: 'rgba(42,42,42,0.05)', marginBottom:15, height:14,lineHeight:14,alignItems: 'center',justifyContent: 'center', backgroundColor: "#FF6A6A", fontSize: 10, color:"#fff", paddingHorizontal: 3,borderRadius: 10,}}>{"爱的供养"}</HiText>},
    ]}
  />
  <Tabs
    values={[
        ...
        return (
          <View 
            ...
            onClick={e => {
              console.log('做点什么')
              // 必要：保留tabProps事件
              params.tabProps.onClick && params.tabProps.onClick(e);
            }}
          >
            <UImage
              src={
